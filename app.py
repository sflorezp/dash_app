...
# os.chdir('C:/Users/Garul/DS4A_workspace/Cases/extended_case_4_student')

engine = create_engine('postgresql://postgres:nRIbHmO9EyIISGIp5i3w@psql-instance.ctpdciwvjn4h.us-east-2.rds.amazonaws.com/strategy')
...
def filter_df(df, exchange, leverage, start_date, end_date):
    df_filt = df[(df['Exchange']==exchange) & (df['Margin']==int(leverage)) & (df['Entry time']>=start_date) & (df['Entry time']<=end_date)]
    return df_filt
            ...
            html.Div(
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                className="two columns w3-container",
                                children=[
                            ...
                            html.Div(
                                className="three columns w3-container",
                                children=[
...
def update_range_date(value):
    df_filt    = df[df['Exchange']==value]
    start_date = df_filt['Entry time'].min()  
    end_date = df_filt['Entry time'].max()
...
def update_monthly(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    dff['Entry time']   = pd.to_datetime(dff['Entry time'])
    dff['YearMonth'] = pd.to_datetime(dff['Entry time'].apply(lambda x: str(x)[0:7])+'-01')
...
def update_daily_btc_portfolio_balance(start_date, end_date, exchange, leverage):
    dff = df[(df['Entry time']>=start_date) & (df['Entry time']<=end_date)]
    dff['Entry time']   = pd.to_datetime(dff['Entry time'])
    dff['Daily'] = pd.to_datetime(dff['Entry time'].apply(lambda x: str(x)[0:10]))
    daily_sum_BTC = dff.groupby('Daily', as_index=False)['BTC Price'].max()       
    
    trace_btc       = go.Scatter(x=daily_sum_BTC["Daily"], y=daily_sum_BTC["BTC Price"])
    
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    trace_portfolio = go.Scatter(x=dff["Entry time"], y=dff["Exit balance"])
    return (
...
def update_pnl_types(start_date, end_date, exchange, leverage):
    dff = filter_df(df, exchange, leverage, start_date, end_date)  

    return {
            'data' : [
                    go.Bar(
                            x = dff[dff['Trade type'] == i]['Entry time'],
                            y = dff[dff['Trade type'] == i]['Pnl (incl fees)'],
                            name = i
                            ) for i in dff['Trade type'].unique()
                    ],
